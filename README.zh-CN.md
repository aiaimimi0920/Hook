...
- 可选的 Loom OCR（`Ctrl+2` 始终重新识别所选贴图并复制全文，`Alt+2` 显示或隐藏按原文字号缩放的结果并启用点击复制，`Ctrl+E` 切换贴图工具栏且不会关闭 OCR 点击态）；工具栏中可扩展的 `OCR` 主菜单当前提供“复制全文”，只复制缓存结果，点击 OCR 块则只复制单块文本并显示贴图内通知；
- 所有贴图与 Art 块的通知都绑定到对应贴图区域，在右上角按时间堆叠显示，点击单条即可关闭；
- 同一 `Ctrl+2` 入口提供本地二维码/条码识别，结果可在属性面板中复制、明确打开 HTTP(S) 地址，并通过 `recognized_url`、`recognized_text`、`recognized_codes` 输出端口连接工作流；
